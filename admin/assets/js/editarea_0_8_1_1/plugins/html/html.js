/**
 * @version $Id: html.js 5 2010-11-05 20:59:12Z elkuku $
...
//				return parent.editAreaLoader.get_button_html('test_but', 'test.gif', 'test_cmd', false, this.baseURL);
			case "html_select":
				html= "<select id='html_select' onchange='javascript:editArea.execCommand(\"html_select_change\")' fileSpecific='no'>"
					+"			<option value='-1'>{$html_select}</option>"
...
//			return true;
	...
	,execCommand: function(cmd, param){
		// Handle commands
		switch(cmd){
			case "html_select_change":
				var val= document.getElementById("html_select").value;
				if(val!=-1)
				parent.editAreaLoader.insertTags(editArea.id, "<"+val+">", "</"+val+">");
				document.getElementById("html_select").options[0].selected=true; 
				return false;
			case "test_cmd":
